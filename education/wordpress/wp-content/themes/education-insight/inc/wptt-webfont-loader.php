...
if ( ! class_exists( 'Education_Insight_WPTT_WebFont_Loader' ) ) {
	...
	class Education_Insight_WPTT_WebFont_Loader {
		...
		public function get_remote_url_contents() {
			...
			$education_insight_contents = wp_remote_retrieve_body( $response );

			return $education_insight_contents;
		}
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $education_insight_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $education_insight_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$education_insight_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$education_insight_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$education_insight_font_family = trim( str_replace( array( "'", ';' ), '', $education_insight_font_family ) );
					$education_insight_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $education_insight_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $education_insight_font_family ] ) ) {
					$result[ $education_insight_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$education_insight_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$education_insight_font_family_url = $this->get_absolute_path( $education_insight_font_family_url );

					$result[ $education_insight_font_family ][] = $education_insight_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $education_insight_font_family ] = array_flip( array_flip( $result[ $education_insight_font_family ] ) );
			}
			return $result;
		}
