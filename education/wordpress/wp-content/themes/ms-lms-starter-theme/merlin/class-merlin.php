...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		$this->base_path              = MS_LMS_STARTER_THEME_DIR;
...
//		wp_safe_redirect( admin_url( 'themes.php?page=merlin') );
	...
	protected function welcome() {
		...
		<div class="merlin__content--transition">
			...
			<?php echo '<img src="https://masterstudy.stylemixthemes.com/starter-demo-files/logo@2x.png" alt="Logo Starter LMS" class="starter-lms-logo-merlin">' ?>
		...
		<footer class="merlin__content__footer">
			...
			$cancel_link = '';
			if('Cancel' == $no) {
				$cancel_link = admin_url( '/' );
			}
			else {
				$cancel_link = wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '/' );
			}
			?>
			<a href="<?php echo esc_url( $cancel_link ); ?>" class="starter-lms-merlin merlin__button merlin__button--skip"><?php echo esc_html( $no ); ?></a>
			<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="starter-lms-merlin merlin__button merlin__button--next merlin__button--proceed merlin__button--colorchange"><?php echo esc_html( $start ); ?></a>
	...
	protected function license() {
		...
		<div class="merlin__content--transition">
			...
			<?php echo '<img src="https://stylemixthemes.com/starter-theme/demo-files/logo@2x.png" alt="Logo Starter LMS" class="starter-lms-logo-merlin">' ?>
	...
	protected function plugins() {
		...
		<div class="merlin__content--transition">
			...
			<?php if ( $count ) { ?>
				<a id="merlin__drawer-trigger" class="merlin__button merlin__button--knockout"></a>
			<?php } ?>
	...
	protected function ready() {
        ...
        $lms_btn = $strings['lms-big-button'];
		...
		<footer class="merlin__content__footer merlin__content__footer--fullwidth <?php echo esc_attr( $links_class ); ?>">
            ...
            <a href="<?php echo esc_url( home_url ('/'). 'wp-admin/admin.php?page=stm-lms-wizard/' ); ?>" class="merlin__button merlin__button--blue merlin__button--fullwidth merlin__button--popin lms"><?php echo esc_html( $lms_btn ); ?></a>
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Demo Content', 'merlin-wp' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Elementor Widgets', 'merlin-wp' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Theme Options', 'merlin-wp' ),
					...
					<label for="default_content_<?php echo esc_attr( $slug ); ?>">
						<i></i><span><?php
							if ( 'after_import' == $slug ) {
								echo esc_html( 'LMS Options' );
							}
							else if ( 'content' == $slug ) {
								echo esc_html( 'Demo Content' );
							}
							else if ( 'widgets' == $slug ) {
								echo esc_html( 'Elementor Widgets' );
							}
							else if ( 'options' == $slug ) {
								echo esc_html( 'Theme Settings' );
							} else {
