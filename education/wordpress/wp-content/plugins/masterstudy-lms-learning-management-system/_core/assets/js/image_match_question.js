    ...
    function init_sortable() {
      ...
      var _loop = function _loop(x) {
        $('.stm_lms_question_image_match').each(function () {
          $('.container_' + [x] + '.may_sort, .answer_' + [x]).sortable({
            connectWith: ['.answer_' + [x], '.container_' + [x]],
            appendTo: document.body,
            helper: 'clone',
            over: function stop(event, ui) {
              $(ui.placeholder).closest('.answer_' + [x]).removeClass('empty');
            },
            out: function start(event, ui) {
              var parent_item = $(ui.placeholder).closest('.answer_' + [x]);
            ...
            receive: function receive(event, ui) {
              var $parent = $(this);
              var $donor = $(ui.sender);
              $parent.closest('.answer_' + [x]).removeClass('empty');
              $parent.closest('.container_' + [x]).removeClass('empty');
              setTimeout(function () {
                if ($donor.html().length < 1) {
                  $donor.closest('.answer_' + [x]).addClass('empty');
                }
              }, 0);
              ...
              if ($(ui.sender).hasClass('stm_lms_question_image_match__container')) {
                $answers = $parent.closest('.stm_lms_question_image_match__questions').find('.answer_' + [x]);
              } else {
                $answers = $donor.closest('.stm_lms_question_image_match__questions').find('.answer_' + [x]);
              }
      ...
      for (var x = 1; x < $('.stm-lms-single_question').length + 1; x++) {
        _loop(x);
      }
    }
  });
