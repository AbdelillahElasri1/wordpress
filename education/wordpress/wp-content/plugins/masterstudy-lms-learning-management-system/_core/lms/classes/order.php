...
class STM_LMS_Order {
	...
	public static function _user_orders( $user_id, $offset = 0 ) {
		...
		if ( empty( $user_orders ) ) {
			...
			if ( $q->have_posts() ) {
				while ( $q->have_posts() ) {
					$q->the_post();
					$id      = get_the_ID();
					$posts[] = self::get_order_info( $id, true );
				}
	...
	public static function user_orders() {
		...
		wp_send_json( self::_user_orders( $user_id, $offset ) );
