			...
			if ( isset( $menu_item['id'] ) && ! $this->search_item_in_sortable_menu( $settings, $menu_name, $menu_item['id'] ) ) {
				...
				switch ( $menu_item['id'] ) {
					...
					case 'announcement':
						...
						break;
					case 'bundles':
						$add_element = class_exists( 'STM_LMS_Course_Bundle' ) && $instructor_menu;
						break;
					case 'gradebook':
					...
					case 'my_assignments':
						...
						break;
					case 'certificates':
						$add_element = class_exists( 'STM_LMS_Certificate_Builder' ) && $full_menu;
						break;
					case 'groups':
		...
		return array_map(
			function ( $menu_item ) use ( $user_id ) {
				if ( isset( $menu_item['id'] ) ) {
					switch ( $menu_item['id'] ) {
						case 'assignments':
							$menu_item['badge_count'] = STM_LMS_Instructor_Assignments::total_pending_assignments( $user_id );
							break;
						case 'my_assignments':
