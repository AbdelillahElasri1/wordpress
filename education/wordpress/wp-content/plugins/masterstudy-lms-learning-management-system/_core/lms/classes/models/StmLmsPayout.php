...
class StmLmsPayout extends StmBaseModel{
	...
	public static function init() {
		add_filter('stm_lms_post_types_array', [self::class, 'payout_post_type'], 10, 1);
	...
	public static function columns_content($column_name, $post_ID) {
		...
		if ($column_name == 'status') {
			...
			if(isset($status[0])) {
				echo stm_lms_filtered_output($status[0]);
			}
		}
	...
	public static function generation_payout() {
		...
		$order_items = StmOrderItems::query()
			...
			->where_in("course.`post_type`", ["stm-courses", "product"])
			->where_raw("
				(  
